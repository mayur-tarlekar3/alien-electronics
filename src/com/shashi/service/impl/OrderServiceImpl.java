...
public class OrderServiceImpl implements OrderService {
	...
	public String paymentSuccess(String userName, double paidAmount) {
		...
		if (ordered) {
			...
			if (ordered) {
				...
				final String fTransId = transaction.getTransactionId();
				final double fAmount = transaction.getTransAmount();
				new Thread(() -> {
					MailMessage.transactionSuccess(fUserName, new UserServiceImpl().getFName(fUserName),
							fTransId, fAmount);
				}).start();
	...
	public List<OrderDetails> getAllOrderDetails(String userEmailId) {
		...
		try {
			...
			ps = con.prepareStatement(
					"SELECT  p.pid as prodid, o.orderid as orderid, o.shipped as shipped, p.image as image, p.pname as pname, o.quantity as qty, o.amount as amount, t.time as time FROM orders o, product p, transactions t where o.orderid=t.transid and o.orderid = t.transid and p.pid=o.prodid and t.username=? ORDER BY t.time DESC");
			...
			while (rs.next()) {
				...
				order.setProdImage(rs.getBinaryStream("image"));
	...
	public String markAsDelivered(String orderId, String prodId) {
		String status = "FAILURE";
		...
		try {
			ps = con.prepareStatement("update orders set shipped=2 where orderid=? and prodid=? and shipped=1");
