...
const EditProductModal = ({ isOpen, product, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    data.append('pid', product.prodId);
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/UpdateProductSrv`, data, {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
      <div className="glass-card w-full max-w-lg p-8 shadow-2xl border-white/10">
        <h2 className="text-2xl font-black uppercase tracking-tighter italic mb-6">
          Update <span className="text-indigo-500">Product</span>
        </h2>
        <p className="text-[10px] text-slate-500 uppercase tracking-[0.2em] mb-8 font-black">ID: {product?.prodId}</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="input-group">
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 block">Product Image (Optional)</label>
            <input 
              type="file" 
